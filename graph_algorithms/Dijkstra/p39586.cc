...
void dijkstra(const Graf &g, int node, vector<int> &distance, vector<int> &ways)
{
    distance[node] = 0;
    ways[node] = 1;
    ...
    while (not pq.empty())
    {
        ...
        if (not visited[act_node])
        {
            ...
            for (int i = 0; i < g[act_node].size(); ++i) //goes through all of his adjacent nodes
            {
                ...
                if (distance[next_node] > distance[act_node] + next_cost)
                {
                    ways[next_node] = ways[act_node];
                    distance[next_node] = distance[act_node] + next_cost;
                    pq.push(arch(distance[next_node], next_node));
                }
                else if (distance[next_node] == distance[act_node] + next_cost) ways[next_node] += ways[act_node];
            }
        }
    }
}
...
int main()
{
    ...
    {
        Graf g(nver);
        vector<int> distance = vector<int>(nver, INFINITY); //initially, the distance from x to every node is infinity
        vector<int> ways(nver, 0);
        ...
        int x, y;
        cin >> x >> y;
        dijkstra(g, x, distance, ways);
        if (distance[y] == INFINITY) cout << "no path from " << x << " to " << y << endl;
        else cout << "cost " << distance[y] << ", " << ways[y] << " way(s)" << endl;
    }
}